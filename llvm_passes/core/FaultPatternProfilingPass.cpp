//===- PatternProfilingPass.cpp - Dynamic Instruction Profiling Pass -==//
//
//                     LLFI Distribution
//
...
#include "FaultPatternProfilingPass.h"
...
char LegacyPatternProfilingPass::ID=0;
...
bool LegacyPatternProfilingPass::runOnModule(Module &M) {
  fprintf(stderr," <<<<<<L1egacyPatternProfilingPass>>>>>>>>\n");
  ...
  for (std::map<Instruction*, std::list< int >* >::const_iterator
    ...
    // FunctionCallee profilingfunc_Mani = getLLFILibProfilingFunc_Mani(M);

    // // prepare for the calling argument and call the profiling function
    // std::vector<Value*> profilingarg_Mani(1);
    // const IntegerType* itype_Mani = IntegerType::get(context, 32);

    // //LLVM 3.3 Upgrading
    // IntegerType* itype_non_const_Mani = const_cast<IntegerType*>(itype_Mani);
    // Value* opcode_Mani = ConstantInt::get(itype_non_const_Mani, fi_inst->getOpcode());
    // profilingarg_Mani[0] = opcode_Mani;
    // ArrayRef<Value*> profilingarg_array_ref_Mani(profilingarg_Mani);

    // fi_inst->print(errs()<<" Do Profiling_Mani()\n");
    // CallInst::Create(profilingfunc_Mani, profilingarg_array_ref_Mani,
    //                  "", insertptr);
  }
...
void LegacyPatternProfilingPass::addEndProfilingFuncCall(Module &M) {
...
FunctionCallee LegacyPatternProfilingPass::getLLFILibProfilingFunc(Module &M) {
  LLVMContext &context = M.getContext();
...
FunctionCallee LegacyPatternProfilingPass::getLLFILibProfilingFunc_Mani(Module &M) {
  LLVMContext &context = M.getContext();
  std::vector<Type*> paramtypes(1);
  paramtypes[0] = Type::getInt32Ty(context);
  ...
  FunctionType* profilingfunctype = FunctionType::get(
      Type::getVoidTy(context), paramtypes_array_ref, false);
  FunctionCallee profilingfunc =
      M.getOrInsertFunction("doProfiling_Mani", profilingfunctype);
  return profilingfunc;
}

FunctionCallee LegacyPatternProfilingPass::getLLFILibEndProfilingFunc(Module &M) {
  LLVMContext& context = M.getContext();
...
static RegisterPass<LegacyPatternProfilingPass> X("patternprofilingpass",
                                     "Pattern Profiling pass", false, false);
}
